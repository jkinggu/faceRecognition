...
public class ZkzInterImpl implements ZkzInterface {
	...
	public PageResult getCountByPageCondition(String kaodian, String changci, String startime, Integer currentPage) {
        ...
        Integer totalCount=DBTemplate.getResult(countsb.toString(),new IntegerHandler());//总条数
        
        List<StatisticsCon> scList= DBTemplate.getResult(sb.toString(), new IResultHandler<List<StatisticsCon>>() {
			@Override
	...
	private List<Integer> getCountgetCountBySelect(String km,String sj, String kd, String kc) {
		...
		// 认证通过统计结果
				...
				String wrzString = "select count(distinct zkzdata.id) from zkzdata left join facelog on zkzdata.upersonnum = facelog.sfz where zkzdata.sj1 like '"
						...
						+ "%' and changci = '" + km + "')";				
				list.add(DBTemplate.getResult(tgString, new IntegerHandler()));
				list.add(DBTemplate.getResult(btgString, new IntegerHandler()));
				list.add(DBTemplate.getResult(wrzString, new IntegerHandler()));
				return list;		
	}
