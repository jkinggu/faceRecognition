...
public class reSizeEvent extends MouseAdapter{
    ...
    public reSizeEvent(JFrame jf, VideoCapture  camera){
        ...
        switch(dragType){
            ...
            case DRAG_RIGHT://x,y位置不变，width变化
                jf.setLocation(jfPos.x,
                        jfPos.y);
                jf.setSize(jf.getWidth()+(curPos.x-prePos.x), jf.getHeight());
//                System.out.println("jf.getSize      "+jf.getSize()+"       "+camera);
//	            camera.set(Videoio.CV_CAP_PROP_FRAME_WIDTH,  jf.getSize().getWidth()/2);
//	            camera.set(Videoio.CV_CAP_PROP_FRAME_HEIGHT,  jf.getSize().getHeight());
